...
export async function getTreasuryAddress(
  ...
  globalState.forEach((el: any) => {
    if (el["key"] === key) {
      ret = el["value"]["bytes"];
      ret = algosdk.encodeAddress(Buffer.from(ret, 'base64'))
      return;
    }
  });
  return ret;
 ...
 * @param withoutAllocate To access the memory that stored in >127 bytes
...
export async function appOptinCheck(
  client: Algodv2,
  appId: bigint,
  receiver: string
): Promise<boolean> {
  const ai = await client.accountInformation(receiver).do();
...
export async function submitVAAHeader(
  ...
  let numTxns: number = 0;
  if (numSigs % MAX_SIGS_PER_TXN === 0) {
    numTxns = Math.floor(numSigs / MAX_SIGS_PER_TXN);
  } else {
...
export async function _submitVAAAlgorand(
  ...
  if (
    ...
    if (parsedVAA.get("FromChain") !== CHAIN_ID_ALGORAND) {
      ...
      // for complete transfer especially, it needs access to the asset native storage
      if (meta === "TokenBridge Transfer") {
        const foreignAsaID = await getForeignAssetAlgorand(client, tokenBridgeId, parsedVAA.get("FromChain"),
          parsedVAA.get("Contract"));
        asaID = foreignAsaID ?? BigInt(0)
      }

      // Foreign assets need to have access to the storage
      const foreignResult = await optin(
        client,
        senderAddr,
        tokenBridgeId,
        asaID,
      ...
      foreignLsig = foreignResult.addr

      chainAddr = result.addr;
      txs.unshift(...foreignResult.txs);
      txs.unshift(...result.txs);
    } else {
      const assetId = hexToNativeAssetBigIntAlgorand(parsedVAA.get("Contract")); // ASA ID in native algorand
    ...
    // Foreign assets need to have access to the storage
    // Only when attestation happen (attest token and receive attest)
    if (foreignLsig != "" && meta === "TokenBridge Attest") {
      accts.push(foreignLsig);
    }
  }
  ...
  if (
    ...
    // 3. Native storage
    // 4. Receiver
    accts.push(addr); // this is the receiver
    accts[accts.length - 3] = treasury // treasury address
    accts[accts.length - 2] = foreignLsig // native storage
    let tx = {
      tx: makeApplicationCallTxnFromObject({
        accounts: accts,
        appArgs: [textToUint8Array("completeTransfer"), vaa],
        foreignApps: [Number(bridgeId), safeBigIntToNumber(escrowId)],
        appIndex: safeBigIntToNumber(tokenBridgeId),
        foreignAssets: foreignAssets,
        from: senderAddr,
        onComplete: OnApplicationComplete.NoOpOC,
...
/**
 * Update the bridge fee on algorand (in percentage)
 * Ex: 10% fee means 1000
 * @param client 
 * @param tokenBridgeId 
 * @param senderAddr 
 * @param assetId 
 * @param transferFee Fee when user do the transfer, zero means no fee
...
export async function getEscrowId(
  client: Algodv2,
  tokenBridgeId: bigint,
  assetId: bigint,
  isNFT: boolean
  ...
  let ls: Uint8Array = await decodeLocalState(
    client,
    tokenBridgeId,
    lsa.address(),
    true
...
export async function escrowWithdraw(
  ...
  txs.push({
    tx: makeApplicationCallTxnFromObject({
      ...
      appArgs: [new Uint8Array(Buffer.from("withdraw")), algosdk.encodeUint64(assetId)]
    }),
    signer: null
  })
  ...
  let tx = makeApplicationCallTxnFromObject({
    from: senderAddr,
    onComplete: OnApplicationComplete.NoOpOC,
    suggestedParams: params,
    appIndex: safeBigIntToNumber(escrowId),
    foreignAssets: foreignAssets,
    appArgs: [new Uint8Array(Buffer.from("withdraw")), algosdk.encodeUint64(amount)]
  })
  tx.fee = tx.fee * 2
  txs.push({
    tx: tx,
...
export async function escrowDeposit(
  ...
  // Optin Check
  let isOptin = await appOptinCheck(client, escrowId, senderAddr);
  let txs: any = []
  if (!isOptin) {
    txs.push({
      tx: makeApplicationOptInTxnFromObject({
        appIndex: safeBigIntToNumber(escrowId),
        from: senderAddr,
        suggestedParams: params
      }),
  ...
  const escrowAddr = algosdk.getApplicationAddress(escrowId)
  if (assetId == BigInt(0)) {
    txs.push({
      tx: makePaymentTxnWithSuggestedParamsFromObject({
        from: senderAddr,
  ...
  // Make an app call to token bridge app
  txs.push({
    tx: makeApplicationCallTxnFromObject({
      from: senderAddr,
      onComplete: OnApplicationComplete.NoOpOC,
      ...
      appIndex: safeBigIntToNumber(tokenBridgeId),
      accounts: [lsa.address()],
      appArgs: [new Uint8Array(Buffer.from("deposit")), algosdk.encodeUint64(assetId)]
    }),
    signer: null
  })

  // Make an app call to escrow app
  txs.push({
    tx: makeApplicationCallTxnFromObject({
      from: senderAddr,
      onComplete: OnApplicationComplete.NoOpOC,
